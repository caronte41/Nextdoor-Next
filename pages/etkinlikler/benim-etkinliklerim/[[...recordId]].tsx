...
export default function MyEventsPage({ eventsData, user }) {
  //context
  ...
  const [eventsPageData, setEventsPageData] = useState(eventsData);
  ...
  const onSubmit = async (data) => {
    ...
    data = {
      ...
      eventEndDay: null,
      eventEndHour: null,
      eventDay: formatDateForBackend(data.eventDay),
  ...
  const AddParticipantToEvent = async (eventId) => {
    let data = {
      EventId: eventId,
      profileId: user.profiles.find((obj) => obj.profileType === "0").id,
      ParticipationStatus: ParticipationStatusEnum.Interested,
    };
    await addParticipantToEvent(data)
      .then((res) => {
        setEventsPageData(eventsPageData);
      })
      .catch((err) => exceptionHandler(err));
  };
  ...
  return (
    <div className="flex flex-col gap-4 mt-8 items-start h-screen">
      ...
      <div className="flex gap-4 items-center">
        <Tabs defaultValue="host" className="w-[400px]">
          <TabsList>
            <TabsTrigger value="host">Sahibi</TabsTrigger>
            <TabsTrigger value="going">Gittiğim</TabsTrigger>
            <TabsTrigger value="interested">İlgilendiğim</TabsTrigger>
        ...
        <Dialog>
          <Form {...form}>
            <DialogTrigger asChild>
              <Button_Custom variant={"outline"}>Etkinlik Yarat</Button_Custom>
            </DialogTrigger>
      ...
      <div>
        {eventsData.map((event, index) => (
          ...
          >
            ...
            <Button_Custom
              disabled={event.isUserParticipant}
              onClick={() => AddParticipantToEvent(event.id)}
            >
              İlgileniyorum
            </Button_Custom>
          </Card>
...
MyEventsPage.Layout = MainLayout;
